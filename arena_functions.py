...
def get_champ(screen_capture, name_pos: Vec4, shop_pos: int, shop_array: list) -> str:
...
def get_shop() -> list:
    ...
    screen_capture = ocr._grab(screen_coords.SHOP_POS.get_coords())
    shop: list = []
    thread_list: list = []
...
def empty_slot() -> int:
    ...
    for slot, positions in enumerate(screen_coords.BENCH_HEALTH_POS):
        screen_capture = ocr._grab(positions.get_coords())
